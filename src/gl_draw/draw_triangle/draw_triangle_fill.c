...
/*   Updated: 2024/03/19 17:17:37 by mchenava         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <lite_gl.h>

void	draw_tri_action_finish(t_gl_context *c, t_draw_tri_vars *vars)
{
	set_vec4(&vars->builtins.gl_frag_coord, (float [4]){vars->x, vars->y,
		vars->z, vars->tmp2});
...
void	draw_tri_action_next(t_gl_context *c, t_draw_tri_vars *vars,
		t_gl_vertex **v, unsigned int provoke)
{
	...
	while (i < c->vs_output.size)
	{
		...
		{
			vars->tmp = vars->alpha * vars->perspective[i] + vars->beta
				* vars->perspective[i + GL_MAX_VERTEX_OUTPUT_COMPONENTS]
				+ vars->gamma * vars->perspective[i
				+ GL_MAX_VERTEX_OUTPUT_COMPONENTS * 2];
			vars->fs_input[i] = vars->tmp / vars->tmp2;
		}
		else if (c->vs_output.interpolation[i] == NOPERSPECTIVE)
			vars->fs_input[i] = vars->alpha * v[0]->vs_out[i] + vars->beta
				* v[1]->vs_out[i] + vars->gamma * v[2]->vs_out[i];
		else
			vars->fs_input[i] = vars->vs_output[provoke * c->vs_output.size
				+ i];
		i++;
	}
...
void	draw_tri_action(t_gl_context *c, t_draw_tri_vars *vars, t_gl_vertex **v,
		unsigned int provoke)
{
	if ((vars->alpha > 0 || line_func(&vars->lines[1], vars->h[0].x,
				vars->h[0].y) * line_func(&vars->lines[1], -1, -2.5) > 0)
		&& (vars->beta > 0 || line_func(&vars->lines[2], vars->h[1].x,
				vars->h[1].y) * line_func(&vars->lines[2], -1, -2.5) > 0)
		&& (vars->gamma > 0 || line_func(&vars->lines[0], vars->h[2].x,
				vars->h[2].y) * line_func(&vars->lines[0], -1, -2.5) > 0))
	{
		vars->tmp2 = vars->alpha * (1 / v[0]->screen_space.w) + vars->beta * (1
				/ v[1]->screen_space.w) + vars->gamma * (1
				/ v[2]->screen_space.w);
		vars->z = vars->alpha * vars->h[0].z + vars->beta * vars->h[1].z
