...
/*   Updated: 2023/04/24 13:11:40 by  mchenava        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <lite_gl.h>

t_cvector_gl_vertex	*cvec_gl_vertex_heap(size_t size, size_t capacity)
{
	t_cvector_gl_vertex	*vec;

	vec = (t_cvector_gl_vertex *)malloc(sizeof(t_cvector_gl_vertex));
	if (!vec)
	{
		ft_assert(vec != NULL);
		return (NULL);
...
t_cvector_gl_vertex	*cvec_init_gl_vertex_heap(t_gl_vertex *vals, size_t num)
{
	t_cvector_gl_vertex	*vec;
...
int	cvec_extend_gl_vertex(t_cvector_gl_vertex *vec, size_t num)
{
	...
	if (vec->capacity < vec->size + num)
	{
		...
		tmp = (t_gl_vertex *)ft_realloc(vec->a, sizeof(t_gl_vertex) * tmp_sz,
				sizeof(t_gl_vertex) * vec->capacity);
		if (!tmp)
		{
			ft_assert(tmp != NULL);
			return (0);
