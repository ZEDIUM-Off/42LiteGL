...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   cvec_glVertex_Array_copy.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By:  mchenava < mchenava@student.42lyon.fr>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/03 12:40:32 by  mchenava         #+#    #+#             */
/*   Updated: 2023/04/24 12:41:35 by  mchenava        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_cvector_gl_vertex_array *dest, t_cvector_gl_vertex_array *src)
{
	...
	tmp = (t_gl_vertex_array *)ft_realloc(
			...
			dest->capacity * sizeof(t_gl_vertex_array));
	if (!tmp)
	{
		ft_assert(tmp != NULL);
		return (0);
	...
	dest->a = tmp;
	ft_memmove(dest->a, src->a, src->size * sizeof(t_gl_vertex_array));
	dest->size = src->size;
...
{
	t_cvector_gl_vertex_array	*vec1;
	t_cvector_gl_vertex_array	*vec2;

	vec1 = (t_cvector_gl_vertex_array *)dest;
	vec2 = (t_cvector_gl_vertex_array *)src;
	vec1->a = NULL;
	vec1->size = 0;
	vec1->capacity = 0;
