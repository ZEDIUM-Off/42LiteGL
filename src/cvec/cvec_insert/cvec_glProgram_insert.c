...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   cvec_glProgram_insert.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By:  mchenava < mchenava@student.42lyon.fr>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/07 09:31:17 by  mchenava         #+#    #+#             */
/*   Updated: 2023/04/24 12:44:59 by  mchenava        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <lite_gl.h>

int	cvec_insert_gl_program(
	t_cvector_gl_program *vec, size_t i, t_gl_program a)
{
	...
	if (vec->capacity > vec->size)
	{
		ft_memmove(&vec->a[i + 1], &vec->a[i],
			(vec->size - i) * sizeof(t_gl_program));
		vec->a[i] = a;
	}
	else
	{
...
int	cvec_insert_array_gl_program(
	t_cvector_gl_program *vec, size_t i, t_gl_program *a, size_t num)
{
	t_gl_program	*tmp;
	size_t			tmp_sz;
	...
	ft_memmove(&vec->a[i + num], &vec->a[i],
		(vec->size - i) * sizeof(t_gl_program));
	ft_memmove(&vec->a[i], a, num * sizeof(t_gl_program));
	vec->size += num;
	return (1);
}
